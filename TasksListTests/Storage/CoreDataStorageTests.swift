...
final class CoreDataStorageTests: XCTestCase {
	...
	func test_create_fetch_edit_delete_CRUD() {
		...
		wait(for: [del], timeout: TestConstants.Timeout.medium)
		...
		wait(for: [fetchAfter], timeout: TestConstants.Timeout.medium)
	...
	func test_fetch_sortedByCreationDesc() {
		...
		DispatchQueue.global().asyncAfter(deadline: .now() + TestConstants.Delay.veryShort) { sut.createTask(title: "B", details: nil) { _ in create2.fulfill() } }
		wait(for: [create1, create2], timeout: TestConstants.Timeout.medium)
